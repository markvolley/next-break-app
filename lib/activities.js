...
// Builds a street-level address string from OSM's addr:* tags, e.g.
// "12 Example St". Returns null (never fabricated) unless both a house
// number and street name are actually present in the source data.
  ...
  try {
    ...
    for (const el of elements) {
      ...
      results.push({
        ...
        suburb: el.tags?.['addr:suburb'] || el.tags?.['addr:city'] || el.tags?.['addr:town'] || null,
        mapUrl: buildGoogleMapsUrl(name, lat, lon)
      });
    }
...
// Stay tab: real, named accommodation near a hometown — same OSM-sourced,
// not-live-availability pattern as findRealRestaurants above. Booking.com's
// real listings/Demand API needs its own partner approval (see
// lib/booking.js), so this fills the same role real restaurant names do:
// genuine places worth considering, not fabricated ones, with a real
// Booking.com search link (already built) for anyone who wants to actually
// check availability and book.
const STAY_SEARCH_RADIUS_M = 15000;
const STAY_LIMIT_RAW = 40;

// OSM's tourism=* value for each accommodation type, with a human label.
// Deliberately excludes tourism=apartment — OSM uses that tag for both
// individual short-let apartments AND large residential apartment blocks
// with no tourism meaning at all, so it's too noisy to trust here.
const STAY_TOURISM_TYPES = [
...
function buildStayOverpassQuery(lat, lon) {
  const clauses = STAY_TOURISM_TYPES.flatMap(t => [
    `node["tourism"="${t.value}"](around:${STAY_SEARCH_RADIUS_M},${lat},${lon});`,
    `way["tourism"="${t.value}"](around:${STAY_SEARCH_RADIUS_M},${lat},${lon});`
  ]).join('\n');
  return `[out:json][timeout:25];(\n${clauses}\n);out center ${STAY_LIMIT_RAW};`;
}

/** Real, named accommodation near a hometown, sourced live from
 * OpenStreetMap — not live availability or bookable, just real places
 * worth considering. Returns [] (never fabricated) if geocoding fails or
 * nothing's mapped nearby. Same `seed` behaviour as findRealRestaurants. */
export async function findRealStays({ hometown, limit = 6, seed, fetchImpl = fetch }) {
  const coords = await geocodeHometown({ hometown, fetchImpl });
  if (!coords) return [];
  ...
  try {
    const res = await fetchImpl(OVERPASS_API, {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain', 'User-Agent': USER_AGENT },
      body: buildStayOverpassQuery(coords.lat, coords.lon)
    });
    if (!res.ok) {
      const body = await res.text().catch(() => '');
      console.error(`[activities] overpass (stays) HTTP ${res.status} — ${body.slice(0, 200)}`);
      return [];
    }
    const json = await res.json();
    const elements = Array.isArray(json.elements) ? json.elements : [];
    ...
    for (const el of elements) {
      ...
      if (lat == null || lon == null) continue;
      seen.add(name);
      const match = STAY_TOURISM_TYPES.find(t => el.tags?.tourism === t.value);
      results.push({
        source: 'osm',
        title: name,
        stayType: match?.label || 'Accommodation',
        address: formatOsmAddress(el.tags),
        suburb: el.tags?.['addr:suburb'] || el.tags?.['addr:city'] || el.tags?.['addr:town'] || null,
        mapUrl: buildGoogleMapsUrl(name, lat, lon)
      });
    ...
    const seedNum = seed != null ? hashString(String(seed) + hometown + '|stay') : hashString(hometown + '|stay');
    return seededShuffle(results, seedNum).slice(0, limit);
  } catch (e) {
    console.error('[activities] overpass (stays) fetch threw:', e.message);
    return [];
  }
}
