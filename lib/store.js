...
function defaultData() {
  return { users: {}, unlocks: {}, accounts: {}, sessions: {}, passwordResets: {} };
}
...
export function createAccount(email, { passwordHash = null, googleSub = null, termsAcceptedAt = null, termsVersion = null } = {}) {
  const data = readRaw();
  const key = email.toLowerCase();
  data.accounts[key] = { email: key, passwordHash, googleSub, termsAcceptedAt, termsVersion, createdAt: new Date().toISOString() };
  writeRaw(data);
...
export function upsertGoogleAccount(email, googleSub, { termsAcceptedAt = null, termsVersion = null } = {}) {
  const data = readRaw();
  const key = email.toLowerCase();
  if (data.accounts[key]) {
    if (!data.accounts[key].googleSub) data.accounts[key].googleSub = googleSub;
  } else {
    data.accounts[key] = { email: key, passwordHash: null, googleSub, termsAcceptedAt, termsVersion, createdAt: new Date().toISOString() };
  }
  writeRaw(data);
  return data.accounts[key];
}
...
export function setAccountProfile(email, { displayName, avatarUrl } = {}) {
  const data = readRaw();
  const key = email.toLowerCase();
