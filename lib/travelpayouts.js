...
async function fetchPricesForDates({ token, origin, destination, departureAt, returnAt, currency, fetchImpl = fetch }) {
  const common = { origin, destination, departure_at: departureAt, return_at: returnAt, currency: (currency || 'usd').toLowerCase(), token };
...
export function buildLiveSearchUrl({ origin, destination, departDate, returnDate, marker, currency }) {
...
// Picks extra destinations to backfill with. Seeded independently from
// pickCandidates (a different seed suffix) so the backfill order doesn't
// just mirror the real-fare search order, and excludes anything already
// shown as a real fare so a destination never appears twice on one break.
export function pickBackfillDestinations(brk, { excludeIatas = new Set(), count = BACKFILL_MINIMUM } = {}) {
...
export function withBackfill(picked, { origin, brk, marker, currency, minimum = BACKFILL_MINIMUM } = {}) {
  ...
  const extras = pickBackfillDestinations(brk, { excludeIatas, count: need });
