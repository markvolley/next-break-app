...
export async function findRealDeals({ token, marker, origin, currency, brk, limit = 3, batchSize = 12, fetchImpl = fetch }) {
  ...
  // South East Asia fills the deal slots first (cheapest SEA fare first),
  // and only once SEA options are exhausted do other continents backfill
  // the remaining slots (again cheapest first). So a break with 3+ real SEA
  // fares shows SEA exclusively; a break with only 1 SEA fare shows that
  // one plus the 2 next-cheapest fares from anywhere else.
  const sea = found.filter(d => d.region === 'SEA').sort((a, b) => a.price - b.price);
  const other = found.filter(d => d.region !== 'SEA').sort((a, b) => a.price - b.price);
  return [...sea, ...other].slice(0, limit);
}
