...
// Not a filter — every fare that fits the break's dates gets shown, since
// requiring a minimum trip length on top of month-granularity search and an
// already-sparse real-fare cache made "no results" the common case. Instead
// this labels each one honestly so the frontend can show a "quick trip"
// flag rather than pretending a 1-night Bali fare and a 4-night one are the
// same kind of deal. Thresholds scale with how far the destination is: a
// short domestic hop earns its keep sooner than a long-haul flight does.
  ...
  const results = await Promise.all(
    candidates.map(async dest => {
      try {
        ...
        // The API returns fares sorted by price — take the cheapest one
        // that actually fits the break's real dates, not just the
        // cheapest overall (which might depart or return on a totally
        // different week).
        ...
        return {
          ...
          nights,
          isQuickTrip: nights < minNightsFor(dest),
