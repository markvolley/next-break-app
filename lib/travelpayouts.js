...
// "sized to fit each break exactly" — so anything that doesn't reasonably
// fit the break's real day range gets dropped in fitsBreak() below, same
// as if no fare had been found at all. A 1-day grace on each side allows
// for overnight/red-eye flights and timezone rounding without being loose
// enough to show a fare that leaves you stranded past your last day off.
const BREAK_FIT_TOLERANCE_DAYS = 1;
...
function fitsBreak(departureAt, returnAt, brk) {
  ...
  const earliestDep = dateOnly(new Date(brk.start.getTime() - BREAK_FIT_TOLERANCE_DAYS * 86400000));
  const breakEnd = dateOnly(brk.end);
  const latestRet = dateOnly(new Date(brk.end.getTime() + BREAK_FIT_TOLERANCE_DAYS * 86400000));
  return dep >= earliestDep && dep <= breakEnd && ret <= latestRet;
}
