...
// Uses grouped_prices with group_by=departure_at rather than the simpler
// prices_for_dates — prices_for_dates' `limit` returns the N cheapest fares
// for the whole month regardless of which day they're on, which in
// practice means every result clusters on whichever 2-3 days happened to
// be cheapest, with the rest of the month invisible to us even though
// popular routes (Bali, etc.) really do have cached fares most days.
// group_by=departure_at instead returns one (the cheapest) fare *per
// calendar day* — real day-by-day coverage across the month, which is what
// actually gives fitsBreak() a fair shot at finding something that lands
// inside the break.
async function fetchPricesForDates({ token, origin, destination, departureAt, returnAt, currency, fetchImpl = fetch }) {
  ...
  const url = `${API_BASE}/aviasales/v3/grouped_prices?${params.toString()}`;
  ...
  return Object.values(json.data); // keyed by departure date -> flatten to a plain list
}
