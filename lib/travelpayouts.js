...
// A fare fitting inside the break's dates isn't the same as being worth
// the trip — 1 night in Bali or 2 nights in Honolulu clears fitsBreak but
// isn't a real deal, it's mostly a flight. Minimum nights scale with how
// far the destination actually is, since a short domestic hop earns its
// keep at 2 nights in a way a long-haul flight doesn't: domestic 2+,
// medium-haul (South East Asia) 3+, long-haul (everything else
// international) 4+.
  ...
  const results = await Promise.all(
    candidates.map(async dest => {
      try {
        ...
        // actually fits the break's real dates AND is long enough to be
        // worth the trip for how far this destination is, not just the
        // cheapest overall (which might depart on a totally different week,
        // or technically fit but only be a single night away).
        const best = data.find(f =>
          fitsBreak(f.departure_at, f.return_at, brk) &&
          nightsBetween(f.departure_at, f.return_at) >= minNightsFor(dest)
        );
        if (!best) return null;
        return {
          source: 'real',
