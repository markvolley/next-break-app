...
export const REAL_DESTINATIONS = [
  { name: 'Bali, Indonesia', iata: 'DPS', blurb: 'Beaches, surf breaks and cheap luxury villas.', domestic: false, region: 'SEA' },
  { name: 'Phuket, Thailand', iata: 'HKT', blurb: 'Island hopping, street food, warm water year-round.', domestic: false, region: 'SEA' },
  ...
  { name: 'Singapore', iata: 'SIN', blurb: 'Easy stopover city with great food scene.', domestic: false, region: 'SEA' },
  ...
  { name: 'Bangkok, Thailand', iata: 'BKK', blurb: 'Street food capital, temples, nightlife.', domestic: false, region: 'SEA' },
  { name: 'Ho Chi Minh City, Vietnam', iata: 'SGN', blurb: 'Cheap eats, coffee culture, history.', domestic: false, region: 'SEA' },
 ...
 * (queried concurrently), then picks a fixed structure: cheapest domestic
 * fare, cheapest South East Asia fare, cheapest other-international fare —
 * in that order. A bucket with no real fare is simply skipped, so a break
 * can return fewer than `limit` results. Destinations with no recent
 * cached search just come back empty — that's expected, not an error.
 */
export async function findRealDeals({ token, marker, origin, currency, brk, limit = 3, batchSize = REAL_DESTINATIONS.length, fetchImpl = fetch }) {
  ...
  // Fixed structure: 1 domestic, 1 South East Asia, 1 other-international —
  // in that order — each the cheapest real fare found in its bucket. If a
  // bucket has no real fare, that slot is simply skipped rather than
  // backfilled with something from another bucket (no fake substitutes).
  const cheapest = list => list.length ? list.sort((a, b) => a.price - b.price)[0] : null;
  const domestic = cheapest(found.filter(d => d.domestic));
  const sea = cheapest(found.filter(d => !d.domestic && d.region === 'SEA'));
  const otherIntl = cheapest(found.filter(d => !d.domestic && d.region !== 'SEA'));
