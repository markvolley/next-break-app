...
// Fixed domestic -> SEA -> other-international priority order, shared by
// selectDeals (real fares) and withBackfill (no-price search-only cards)
// below, so a page that's part real, part backfill still reads as a
// consistent domestic/SEA/international layout rather than a random
// assortment. Defined once, up here, so both functions use the exact same
// categorisation logic instead of two copies drifting apart.
...
// How many destinations to guarantee per category — 2 domestic, 2 SEA, 2
// other-international, always 6 total. Real fares fill each category's
// slots first (cheapest/best-matching within that category); anything a
// category is still short on gets filled by withBackfill's no-price
// search-only cards, never by borrowing a slot from a different category.
export const DEALS_PER_CATEGORY = 2;

/**
 * Turns the full set of real fares found for a break (from
 * `fetchAllRealFares`) into the deals shown on the dashboard. Picks up to
 * `perCategory` real fares (best-matching/cheapest first) from EACH of
 * domestic, SEA, and other-international, strictly separately — a category
 * with plenty of real fares never spills over and crowds out a thinner
 * one, and a category with fewer than `perCategory` real fares is simply
 * left short (never padded with a fake fare; withBackfill is what tops a
 * short category up with real-destination-but-no-price cards). This only
 * ever returns real, genuinely-found fares; it never invents anything.
 *
 ...
 */
export function selectDeals(found, { limit = DEALS_PER_CATEGORY * CATEGORY_ORDER.length, perCategory = DEALS_PER_CATEGORY, profile = null } = {}) {
  const picked = [];
  const pickedIatas = new Set();

  for (const cat of CATEGORY_ORDER) {
    const bucket = found.filter(d => destCategory(d) === cat);
    for (let i = 0; i < perCategory; i++) {
      const available = bucket.filter(d => !pickedIatas.has(d.iata));
      const best = pickBest(available, profile);
      if (!best) break; // this category is short on real fares — leave it to withBackfill
      picked.push(best);
...
// fare per category, or none at all. Rather than leave that as a dead end
// or a lopsided list, every break always shows DEALS_PER_CATEGORY (see
// above) destinations for EACH of domestic/SEA/international — real fares
// first, and for any category still short, extra real destinations with NO
// price shown at all — just a name and a link to a live Aviasales search
// for the break's own real dates (never a mismatched cached date). This
// never fabricates a price, which this app has never done, and it still
// earns commission if the user books, since the affiliate marker rides
// along on the live-search link itself — commission is tracked by the
// click-through reaching Aviasales with the marker attached, not by
// whether a price was shown here first.
...
/**
 * Tops up an already-selected deals list so EVERY category (domestic, SEA,
 * international) has `perCategory` destinations, using no-price "optional
 * destination" cards for whichever categories are still short, always
 * using the break's own real dates. Each backfill card carries
 * `source: 'search-only'` so the frontend can render it distinctly — no
 * price line, a "no deal found yet" tag, and a button that goes to a live
 * search instead of a specific booked fare. A no-op (returns `picked`
 * unchanged, same array reference) once every category already has
 * `perCategory` real fares.
 *
 * Unlike the old single combined "top up to N total" behaviour, shortfall
 * is computed per category — a category that already has plenty of real
 * fares never "donates" a slot to a thinner one, and a category with zero
 * real fares still gets its full `perCategory` real-destination cards, not
 * none. The final list — real fares plus backfill cards together — is
 * always ordered domestic, then SEA, then international, matching the same
 * rule `selectDeals` uses for real fares.
 */
export function withBackfill(picked, { origin, brk, marker, currency, perCategory = DEALS_PER_CATEGORY } = {}) {
  if (!origin || !brk) return picked;

  const countByCategory = {};
  for (const d of picked) countByCategory[destCategory(d)] = (countByCategory[destCategory(d)] || 0) + 1;

  const missingCategories = [];
  for (const cat of CATEGORY_ORDER) {
    const shortfall = Math.max(0, perCategory - (countByCategory[cat] || 0));
    for (let i = 0; i < shortfall; i++) missingCategories.push(cat);
  }
  if (!missingCategories.length) return picked;

  const excludeIatas = new Set(picked.map(d => d.iata));
  const extras = pickBackfillDestinations(brk, { excludeIatas, categories: missingCategories, count: missingCategories.length });
