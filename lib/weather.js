...
export const DESTINATION_COORDS = {
  ...
  // Added alongside the destination-list expansions in lib/travelpayouts.js
  // — every REAL_DESTINATIONS entry needs coordinates here or it silently
  // gets no weather at all (getWeatherForDate returns null with no coords).
  CBR: { lat: -35.3084, lon: 149.1244 },  // Canberra
  NTL: { lat: -32.7952, lon: 151.8344 },  // Newcastle
  HBA: { lat: -42.8361, lon: 147.5103 },  // Hobart
  BNE: { lat: -27.3942, lon: 153.1218 },  // Brisbane
  PER: { lat: -31.9403, lon: 115.9669 },  // Perth
  DRW: { lat: -12.4083, lon: 130.8726 },  // Darwin
  ASP: { lat: -23.8067, lon: 133.9018 },  // Alice Springs
  LST: { lat: -41.5453, lon: 147.2143 },  // Launceston
  MCY: { lat: -26.6033, lon: 153.0914 },  // Sunshine Coast
  PPP: { lat: -20.4950, lon: 148.5556 },  // Whitsundays (Proserpine)
  KUL: { lat: 2.7456, lon: 101.7099 },    // Kuala Lumpur
  MNL: { lat: 14.5086, lon: 121.0198 },   // Manila
  ICN: { lat: 37.4602, lon: 126.4407 },   // Seoul (Incheon)
  DXB: { lat: 25.2532, lon: 55.3657 },    // Dubai
  LON: { lat: 51.4700, lon: -0.4543 },    // London (Heathrow)
...
export async function fetchForecast({ iata, dateISO, fetchImpl = fetch }) {
  ...
  const url = `${FORECAST_API}?latitude=${coords.lat}&longitude=${coords.lon}&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=auto&start_date=${dateISO}&end_date=${dateISO}`;
  try {
    ...
    // A specific-day rain chance for a real forecast — distinct from
    // "rainy season" below, which is a longer-term historical pattern, not
    // a prediction for this exact date.
    const rainChance = json.daily?.precipitation_probability_max?.[0];
    return {
      type: 'forecast', high: Math.round(high), low: Math.round(low), icon, label, month: monthName(dateISO),
      rainLikely: rainChance != null ? rainChance >= 50 : null
    };
  } catch (e) {
...
export async function fetchTypicalWeather({ iata, dateISO, fetchImpl = fetch }) {
  ...
  try {
    const results = await Promise.all(windows.map(async w => {
      const url = `${ARCHIVE_API}?latitude=${coords.lat}&longitude=${coords.lon}&daily=temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=auto&start_date=${w.start}&end_date=${w.end}`;
      const res = await fetchImpl(url);
      if (!res.ok) return null;
      const json = await res.json();
    ...
    for (const d of results) {
      ...
      (d.precipitation_sum || []).forEach(v => { if (v != null) rain.push(v); });
    }
    if (!highs.length || !lows.length) return null;

    const avg = arr => arr.reduce((a, b) => a + b, 0) / arr.length;
    // "Rainy season" is a rough, disclosed heuristic — average daily
    // rainfall (mm) over the same real 3-year historical window used for
    // the typical high/low, not a forecast. >=4mm/day average is roughly
    // the point tropical/monsoon destinations cross from "dry" to "wet"
    // season in their own historical record.
    const avgRainMm = rain.length ? avg(rain) : null;
    return {
      type: 'typical', high: Math.round(avg(highs)), low: Math.round(avg(lows)), icon: '🌡️', label: null, month: monthName(dateISO),
      rainySeason: avgRainMm != null ? avgRainMm >= 4 : null
    };
  } catch (e) {
